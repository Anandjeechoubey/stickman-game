...
const formNameElement = document.getElementById("form-name-field");
...
function submitName() {
  ...
  startGame(formNameElement.value);
}
...
async function resetGame() {
  ...
  updateHighScore();
  introElement.innerText = "Hold down the mouse to stretch out a stick";
  ...
  generatePlatform();
  bridges = [
    { x: GamePlatform[0].x + GamePlatform[0].w, length: 0, rotation: 0 },
  ];
  tree = [];
  generateTree();
  generateTree();
...
function animate(timestamp) {
  ...
  switch (phase) {
    ...
    case "walking": {
      ...
      if (nextPlatform) {
        const maxHeroX =
          nextPlatform.x + nextPlatform.w - CharacterDistanceEdge;
